...
function AssignClassrooms() {
    ...
    useEffect(() => {
        const fetchAssignments = async () => {
            try {
                const response = await axios.get('http://localhost:5001/api/fetchclassrooms');
    ...
    const handleAssign = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5001/api/assignclassrooms', {
    ...
    const handleDelete = async (assignmentId) => {
        try {
            await axios.delete(`http://localhost:5001/api/removeclassroom/${assignmentId}`);

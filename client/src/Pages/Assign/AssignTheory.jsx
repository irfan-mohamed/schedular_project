...
function AssignTheory() {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        const response = await axios.get('http://localhost:5001/api/fetchassignments');
  ...
  const handleAssign = async () => {
    try {
      await axios.post('http://localhost:5001/api/theoryassignments', {
  ...
  const assignedSubjectIds = assignments.map(assignment => assignment.subjectId[0]?.id);

  const availableSubjects = subjects.filter(subject => subject.subjectType === 'Theory' && !assignedSubjectIds.includes(subject.id));
              ...
              <div className="col-md-6 mb-3">
                ...
                <select id="teacher" className="form-select custom-select" onChange={(e) => setSelectedTeacher(e.target.value)}>
                  ...
                  {teachers.map(teacher => (
                    <option key={teacher.id} value={teacher.id}>{teacher.teacherName}</option>
                  ))}
                </select>
              </div>
            </div>

...
function AssignLab() {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        const response = await axios.get('http://localhost:5001/api/fetchlabs');
  ...
  const handleAssign = async () => {
    try {
      await axios.post('http://localhost:5001/api/labassignments', {
  ...
  const availableSubjects = subjects.filter(subject => subject.subjectType === 'Lab' && !assignedSubjectIds.includes(subject._id));
    ...
    <>
      ...
      <div className="container-fluid">
        <div className="row">
          ...
          <div className="col">
            <h1 className="text-center mt-5 mb-4 custom-heading">Assign Lab</h1>
            ...
            <div className="row mt-4">
              <div className="col">
                ...
                <table className="table">
                  ...
                  <tbody>
                    {assignments.filter(item => item.subjectId[0]?.subjectType === 'Lab')
                      .map((item, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'table-light' : 'table-secondary'}>
                          ...
                          <td>
                            {item.subjectId.map((subject, index) => (
                              subject.subjectName
                            ))}
                          </td>
                          <td>
